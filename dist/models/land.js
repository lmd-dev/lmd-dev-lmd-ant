...
export class Land {
     ...
     */
    initGrid(width, height, landDescription = null) {
        ...
        for (let iRow = 0; iRow < height; ++iRow) {
            for (let iColumn = 0; iColumn < width; ++iColumn) {
                ...
                if (iRow > 0)
                    cell.neighbors.push(this._cells[iRow - 1][iColumn]);
                if (iRow < height - 1)
                    cell.neighbors.push(this._cells[iRow + 1][iColumn]);
                if (iColumn > 0)
                    cell.neighbors.push(this._cells[iRow][iColumn - 1]);
                if (iColumn < width - 1)
                    cell.neighbors.push(this._cells[iRow][iColumn + 1]);
            }
        }
